...
const Survey = () => {
    ...
    useEffect(() => {
        let tempData = fetchData('/surveys');
    ...
    const onFinish: FormProps<FieldType>['onFinish'] = (values) => {
        ...
        let tempSubmit;
        if (isEditing) {
            tempSubmit = updateRecord('/surveys', values);
        } else {
            tempSubmit = addRecord('/surveys', values);
        }
    ...
    const confirm: PopconfirmProps['onConfirm'] = (record) => {
        ...
        let tempDelete = deleteRecord('/surveys', record.id);
    ...
    return (
        <div className='w-full p-4'>
            <h2 className='text-4xl font-bold ml-6 text-blue-300'>Surveys ({data ? data.length : 0})</h2>
            {/* filters */}
            <div className='flex p-2 m-4 float-right'>
                <UploadXLS url={'/surveys'} />

                <Button className='ml-2 mr-4' type="dashed" icon={<SendOutlined />} size='middle' onClick={() => exportToExcel(data, 'survey')}>Export data</Button>
                <Search placeholder="Search" style={{ width: 200 }} />
                <Button className='ml-2' type='primary' icon={<PlusOutlined />} size='middle' onClick={() => { setIsEditing(false); showModal() }}>Add a new Survey</Button>
            </div>
            {/* data grid */}
            <Table bordered className='ml-4 mr-4' rowKey={'id'} columns={columns} dataSource={data ? data : []} />
            <Modal title={isEditing ? "Update this Survey" : "Create New Survey"} open={isModalOpen} centered onOk={handleOk} onCancel={handleCancel} footer={false}>
                <Form
                    name="surveys"
                    labelCol={{ span: 8 }}
                    wrapperCol={{ span: 16 }}
