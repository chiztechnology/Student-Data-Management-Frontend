...
const Grade = () => {
    ...
    const onFinish: FormProps<FieldType>['onFinish'] = (values) => {
        ...
        let tempSubmit;
        if (isEditing) {
            tempSubmit = updateRecord('/grades', values);
        } else {
            tempSubmit = addRecord('/grades', values);
        }
    ...
    const confirm: PopconfirmProps['onConfirm'] = (record) => {
        ...
        let tempDelete = deleteRecord('/grades', record.id);
    ...
    return (
        <div className='w-full p-4'>
            <h2 className='text-4xl font-bold ml-6 text-blue-300'>Grades ({data ? data.length : 0})</h2>
            {/* filters */}
            <div className='flex p-2 m-4 float-right'>
                <UploadXLS url={'/grades'} />

                <Button className='ml-2 mr-4' type="dashed" icon={<SendOutlined />} size='middle' onClick={() => exportToExcel(data, 'grades')}>Export data</Button>
                <Search placeholder="Search" style={{ width: 200 }} />
                <Button className='ml-2' type='primary' icon={<PlusOutlined />} size='middle' onClick={() => { setIsEditing(false); showModal() }}>Add a new grade</Button>
            </div>
            {/* data grid */}
            <Table bordered className='ml-4 mr-4' rowKey={'id'} columns={columns} dataSource={data ? data : []} />
            <Modal title={isEditing ? "Update this grade" : "Create New grade"} open={isModalOpen} centered onOk={handleOk} onCancel={handleCancel} footer={false}>
                <Form
                    name="grades"
                    labelCol={{ span: 8 }}
                    wrapperCol={{ span: 16 }}
