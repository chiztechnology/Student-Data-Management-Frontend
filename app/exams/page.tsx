...
const Exams = () => {
    ...
    useEffect(() => {
        let tempData = fetchData('/examResults');
    ...
    const onFinish: FormProps<FieldType>['onFinish'] = (values) => {
        ...
        let tempSubmit;
        if (isEditing) {
            tempSubmit = updateRecord('/examResults', values);
        } else {
            tempSubmit = addRecord('/examResults', values);
        }
    ...
    const confirm: PopconfirmProps['onConfirm'] = (record) => {
        ...
        let tempDelete = deleteRecord('/examResults', record.id);
    ...
    return (
        <div className='w-full p-4'>
            <h2 className='text-4xl font-bold ml-6 text-blue-300'>Exam Results ({data ? data.length : 0})</h2>
            {/* filters */}
            <div className='flex p-2 m-4 float-right'>
                <UploadXLS url={'/examResults'} />

                <Button className='ml-2 mr-4' type="dashed" icon={<SendOutlined />} size='middle' onClick={() => exportToExcel(data, 'exams')}>Export data</Button>
